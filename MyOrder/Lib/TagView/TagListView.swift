...
open class TagListView: UIView {
    ...
    @IBInspectable open dynamic var removeImage: UIImage? = nil {
        didSet {
            defer { rearrangeViews() }
            tagViews.forEach {
                $0.removeImage = removeImage
            }
        }
    }
    ...
    func setRows(isRtl: Bool) {
        ...
        for (index, tagView) in tagViews.enumerated() { tagView.frame.size = tagView.intrinsicContentSize
            ...
            if currentRowTagCount == 0 || currentRowWidth + tagView.frame.width > frameWidth {
                ...
                setCurrentRowView(currentRowView: currentRowView,
                                  directionT: directionTransform, currentRow: currentRow)
                tagView.frame.size.width = min(tagView.frame.size.width, frameWidth)
            }
            setUpTagBackGroundView(index: index, directionTransform: directionTransform,
            currentRowWidth: currentRowWidth, tagView: tagView, currentRowView: currentRowView)
            currentRowTagCount += 1
            currentRowWidth += tagView.frame.width + marginX
            setUpAligment(currentRowView: currentRowView, frameWidth: frameWidth, currentRowWidth: currentRowWidth)
        }
        rows = currentRow
        invalidateIntrinsicContentSize()
    }
    func setCurrentRowView(currentRowView: UIView, directionT: CGAffineTransform, currentRow: Int) {
        currentRowView.transform = directionT
        currentRowView.frame.origin.y = CGFloat(currentRow - 1) * (tagViewHeight + marginY)
        rowViews.append(currentRowView)
        addSubview(currentRowView)
    }
    func setUpTagBackGroundView(index: Int, directionTransform: CGAffineTransform,
                                currentRowWidth: CGFloat, tagView: UIView, currentRowView: UIView) {
        ...
        tagBackgroundView.layer.shadowPath = UIBezierPath(roundedRect:
            tagBackgroundView.bounds, cornerRadius: cornerRadius).cgPath
    ...
    func setUpAligment(currentRowView: UIView, frameWidth: CGFloat, currentRowWidth: CGFloat) {
        switch alignment {
    ...
    private func createNewTagView(_ title: String, tagValue: Int) -> TagView {
        let tagView = TagView(title: title)
        tagView.tag = tagValue
        setTagViewProperty(tagView: tagView)
    ...
    func setTagViewProperty(tagView: TagView) {
        ...
        tagView.removeImage = removeImage
    }
    @discardableResult
    open func addTag(_ title: String) -> TagView {
        defer { rearrangeViews() }
        return addTagView(createNewTagView(title, tagValue: 0))
    }
    @discardableResult
    open func addTags(_ titles: [String]) -> [TagView] {
        ...
        for (index, name) in titles.enumerated() {
            tagView = createNewTagView(name, tagValue: index)
            tagViewArray.append(tagView)
        }
        return addTagViews(tagViewArray)
    }
    
    @discardableResult
    open func addTagsColors(_ colorName: [UIColor]) -> [TagView] {
        var tagView: TagView!
        var tagViewArray = [TagView]()
        for (index, color) in colorName.enumerated() {
            tagView = createNewTagView("", tagValue: index)
            tagView.selectedBackgroundColor = color
            tagView.tagBackgroundColor = color
            tagView.setShadow()
//            tagView.borderColor = UIColor.white
//            tagView.selectedBorderColor = UIColor.white
            tagViewArray.append(tagView)
        }
        return addTagViews(tagViewArray)
    }
    ...
    open func insertTag(_ title: String, at index: Int) -> TagView {
        return insertTagView(createNewTagView(title, tagValue: 0), at: index)
    }
    @discardableResult
